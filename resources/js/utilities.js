...
const createHomeScreen = function (continueButton = false, resultButton = false) {
  ...
  const textTitle = document.createTextNode('Simulado DETRAN');
  const hSubTitle = document.createElement('h3');
  const textSubTitle = document.createTextNode('Primeira Habilitação');
  ...
  const textButtonStart = document.createTextNode('Novo simulado');
...
// Função para a opção de marcar a resposta e savá-la posteriormente
